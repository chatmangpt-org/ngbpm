...
ENV VIRTUAL_ENV /opt/ngbpm-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/ngbpm/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/ngbpm/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/ngbpm/ && touch src/ngbpm/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/ngbpm/
...
ENTRYPOINT ["/opt/ngbpm-env/bin/poe"]
CMD ["api"]
